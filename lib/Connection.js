...
Connection.prototype.handleConnect = function() {
  if (!this.inbound) {
    console.log("*******-----Shannon", "sending version")
    this.sendVersion();
  }
  this.emit('connect', {
    conn: this,
...
Connection.prototype.handleDisconnect = function() {
  console.log("*******-----Shannon", "disconnect")
...
Connection.prototype.handleMessage = function(message) {
  if (!message) {
    console.log("*******-----Shannon", "not msg")
    // Parser was unable to make sense of the message, drop it
    return;
  }
 console.log("*******-----Shannon", "msg". message)
  try {
...
Connection.prototype.sendVersion = function() {
  ...
  put.word32le(0);
  console.log("*******-----Shannon", "sendver", put)
  put.write(process.stdout)
...
Connection.prototype.sendMessage = function(command, payload) {
  try {
    ...
    console.log("hex?", magic, this.network.magic)
...
Connection.prototype.parseMessage = function(command, payload) {
  console.log("**********-----Shannon", "parse msg", payload, command)
